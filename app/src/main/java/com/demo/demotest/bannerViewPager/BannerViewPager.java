...
public class BannerViewPager extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    public void setAdapter(BannerPagerAdapter adapter){
        mIndictorCount=adapter.getCount()-1;
    ...
    public void onPageScrollStateChanged(int state) {
        ...
        if(state == ViewPager.SCROLL_STATE_DRAGGING){
            ...
            //0,1,2,0
            if(mCurrentPosition==mIndictorCount)
            {
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        LogSuperUtil.i(Constants.LogTag.theme_recommand,"state="+mViewPagerScrollState+",mIndictorCount="
                +mIndictorCount+",position="+position+",positionOffset="+positionOffset);
        if(position==mIndictorCount-1)
        {
        ...
        if(position==mIndictorCount)//修复点闪了一下（因为最后一个图片翻页时，翻页完毕走了一下position=3,positionOffset=0.0）
        {
            position=0;
        }
